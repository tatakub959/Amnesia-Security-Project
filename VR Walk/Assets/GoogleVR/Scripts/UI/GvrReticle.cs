...
public class GvrReticle : MonoBehaviour, IGvrGazePointer {
  ...
  public void OnGazeStay(Camera camera, GameObject targetObject, Vector3 intersectionPosition,
    ...
    SetGazeTarget(intersectionPosition, isInteractive);
        ...
        {
            if(Time.time - GazeStartTime > GazeOfset && ExecuteEvents.CanHandleEvent<TimeInput>(GazedAt))
            {
                GazeStartTime = -1f;
                ExecuteEvents.Execute(GazedAt, null, (TimeInput handler, BaseEventData data) => handler.HandleTimeInput());
            }
        }
        */
    }
